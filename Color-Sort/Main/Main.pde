...
void draw() {
  if (sortGrid() != 0) {                   // sortGrid both sorts the grid once and returns the number of pixels moved
    ...
    float secondsPassed = (frameCount/60); // for some reason putting this INSIDE the println doesnt work
    println("\n"+round(secondsPassed/166 * 100)+"% complete. Hue: "+hue); // print % and current hue
    indices.shuffle();
  }
  drawGrid();
  hue = (hue + 1) % 100;
...
void drawGrid() {
  for (int i = 0; i < 100; i++) {         // treat grid like a 2D array,
    for (int j = 0; j < 100; j++) {  
      ...
      rect(j*10, i*10, 10, 10);           // pixels are 10x10
    }
  }
}