...
colors = {'obstacle': color(90, 90, 90),
          ...
          'bg': color(200), 
          'cursor': color(170,67,255),
          'reset': color(255,59,89) }
...
def drawScores():
    fill(255, 212, 36)
    if attempts[lvl - 1] < 10:
        rect(20, 20, 280, 150, 30)
    if attempts[lvl - 1] >= 10:
        rect(20, 20, 340, 150, 30)
    fill(50)
    output = ''.join(['#', str(lvl), ':', str(attempts[lvl - 1])])
    textSize(120)
    text(output, 20, 140)
    ...
    for i in range(lvl - 1):
        ...
        fill(255, 212, 36)
        if attempts[i] < 10:
            rect(20, 90 + (120 * n), 135, 100, 20)
        if attempts[i] >= 10:
            rect(20, 90 + (120 * n), 170, 100, 20)
        fill(50)
        textSize(60)
        output = ''.join(['#', str(i + 1), ':', str(attempts[i])])
        text(output, 20, 160 + (120 * n))
...
def resetButtonCheck():
    if mousePressed:
        if get(mouseX,mouseY) in [colors['reset'],color(51)]:
            RT.ongoing = True    
    ...
    if ET.ongoing:
        ...
        if ET.radius > 2000:
            if not ET.switched:
                ...
                swung = False
            ET.opacity -= 5
        if ET.radius < 2000: ET.radius *= 1.1
        if ET.opacity <= 0:  ET.reset()
...
def drawClub():
    if mousePressed and not (get(mouseX,mouseY) in [colors['reset'],51]) and not swung and int(vel.x) == 0 and int(vel.y) == 0 and pos.x > 0:

        for curPoint in range(1, 30):
            target = PVector(pos.x + ((pos.x - mouseX) / (5)) *
                             curPoint, pos.y + ((pos.y - mouseY) / (5)) * curPoint)
